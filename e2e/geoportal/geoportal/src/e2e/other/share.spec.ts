...
test.describe("geoportal share", () => {
  ...
  test("Share buttons add to clipboard a configured link.", async ({
    ...
    // --- First copy ---
    await copyBtn.click();
    const modalMessgae = page.getByText(
      "Link wurde in die Zwischenablage kopiert."
    );
    await expect(modalMessgae).toBeVisible();
    await expect(modalMessgae).toBeHidden();
    ...
    expect(clipboardContent).toMatch(/[?&#]lat=-?\d+(\.\d+)?/);
    expect(clipboardContent).toMatch(/[?&#]lng=-?\d+(\.\d+)?/);
    expect(clipboardContent).toMatch(/(?:[?&#]data=|[?&#]config=)/);
  });
});
